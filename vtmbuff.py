...
CONFIG = {
    ...
    'api': {
        ...
        'products': "MVIP", # Updated based on API response
        'views': "VTMBUFF",
        'username': "VOBA",
        'password': "OZAN1239"
    }
...
TABLE_NAME = "mainview_network_vtmbuff"

# API Endpoints
LOGON_URL = "http://192.168.60.20:15565/cra/serviceGateway/services/MVERESTAPI_VBT1_3940/logon"
DATA_URL = "http://192.168.60.20:15565/cra/serviceGateway/services/MVERESTAPI_VBT1_3940/products/MVIP/views/VTMBUFF/data"
...
def create_table_if_not_exists():
    """Creates the VTMBUFF table if it does not exist."""
...
def fetch_vtmbuff_data():
    """Fetches 'VTMBUFF' data from the API."""
    if not api_token:
        logging.error("Cannot fetch 'VTMBUFF' data without a token.")
        return None
    ...
    try:
        ...
        logging.info("'VTMBUFF' data successfully fetched from API.")
        return api_data
    except requests.exceptions.HTTPError as e:
        if e.response.status_code == 401:
            logging.warning("Token expired or invalid. Attempting re-authentication.")
            return 'reauth'
        logging.error(f"'VTMBUFF' API error: HTTP {e.response.status_code}")
        return None
    except requests.exceptions.RequestException as e:
        logging.error(f"'VTMBUFF' data fetching error: {e}")
        return None

# --- DATA PROCESSING AND STORAGE ---
def save_vtmbuff_data_to_db(data):
    """Processes fetched data and saves it to the PostgreSQL database."""
    global error_count
    logging.info("--- VTMBUFF DATA PROCESSING ---")

    records = data.get('Rows')
    if not records or not isinstance(records, list) or len(records) == 0:
        logging.warning("Invalid or empty VTMBUFF data received. Skipping save.")
        error_count += 1
        return False
    ...
    try:
        ...
        for record in records:
            # Map API keys to database columns based on the document
            # J@SYSTEM comes as a list of dicts, extract the first value
        ...
        conn.commit()
        logging.info(f"{success_count} VTMBUFF records successfully added to the database.")
        error_count = 0
        return True
    ...
    while True:
        try:
            if (datetime.now() - last_run).total_seconds() >= INTERVAL_SECONDS:
                logging.info("\n" + "="*40 + "\n--- VTMBUFF DATA PROCESSING CYCLE ---")
                
                api_data = fetch_vtmbuff_data()
                if api_data == 'reauth':
                    if not get_token():
                ...
                if api_data:
                    save_vtmbuff_data_to_db(api_data)
                else:
                    error_count += 1
