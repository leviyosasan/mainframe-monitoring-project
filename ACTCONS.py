...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('actcons_monitoring.log', encoding='utf-8'),
...
def monitor_actcons():
    ...
    while True:
        try:
            ...
            if data:
                save_to_json(data, 'actcons_monitoring_data.json')
...
def main():
    """Main function to start ACTCONS monitoring"""
    logger.info("🚀 ACTCONS Monitoring Service Starting")
    logger.info("⏰ Monitoring interval: 60 seconds")
    logger.info("💾 Data will be saved to: mainview_network_actcons table")
    logger.info("📄 Backup data will be saved to: actcons_monitoring_data.json")
    logger.info("\n💡 Press Ctrl+C to stop monitoring")
    
    try:
        monitor_actcons()
    except KeyboardInterrupt:
        logger.info("🛑 ACTCONS monitoring service stopped by user")
    except Exception as e:
        logger.error(f"❌ Fatal error in ACTCONS monitoring service: {e}")
